    ...
    <div className="flex flex-wrap justify-center p-5 ">
      {data.map((item, idx) => {
        return (
          <div
            key={idx}
            className={`relative flex flex-col items-center w-96 h-[300px] pt-8 duration-300 group`}
          >
            <div
            ...
            >
              <img
                src={item.image}
                alt={item.name}
                className="w-24 transition-transform duration-300 group-hover:translate-y-[-20px]"
              />
            </div>
            <div className="flex flex-col justify-center items-center h-[100px] w-full absolute bottom-0 px-5">
              ...
              <NavLink className="text-md hover:text-blue-500 transition ">
                {item.description}
              </NavLink>
            </div>
          </div>
